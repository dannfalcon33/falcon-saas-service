  ...
  return (
    <div className="bg-white/[0.03] border border-white/5 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden backdrop-blur-xl">
      <div className="p-4 md:p-8 border-b border-white/5">
        <h2 className="text-xl md:text-2xl font-serif font-bold text-white tracking-tight">Biblioteca de Reportes</h2>
        <p className="text-[10px] md:text-xs text-[#8A9199] font-medium italic mt-1 font-serif tracking-widest uppercase opacity-80 leading-relaxed disabled:opacity-50">
          Documentación técnica de visitas e incidencias resueltas.
        </p>
      </div>

      <div className="md:hidden divide-y divide-white/5">
        {reports.length > 0 ? reports.map((report) => (
          <article key={report.id} className="p-4 space-y-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-[#3D7BFF] shrink-0">
                  <FileText className="w-5 h-5" />
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-bold text-white tracking-tight truncate">{report.title}</p>
                  <p className="text-[10px] text-[#8A9199] font-medium uppercase tracking-widest italic truncate">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {report.file_path && (
                  <button
                    ...
                    className="p-2 border border-[#3D7BFF]/20 bg-[#3D7BFF]/5 rounded-lg text-[#3D7BFF] hover:bg-[#3D7BFF] hover:text-white transition-all"
                  >
            ...
            <div className="p-3 rounded-xl border border-white/5 bg-black/20">
              <div className="flex items-center gap-2 text-white/60">
                ...
                <span className="text-[11px] font-bold">{new Date(report.created_at).toLocaleDateString()}</span>
              </div>
            </div>

            <div className="p-3 rounded-xl border border-white/5 bg-black/20 space-y-2">
              <p className="text-[10px] font-black uppercase tracking-widest text-white/30">Recomendaciones</p>
              {report.recommendations ? (
                <p className="text-[11px] text-[#8A9199] font-medium italic leading-relaxed line-clamp-3">
                  {report.recommendations}
                </p>
              ) : (
                <span className="text-[10px] text-white/20 font-black uppercase tracking-widest italic">N/A</span>
              )}
            </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="p-4 bg-black/20 rounded-xl border border-white/5">
                    <p className="text-xs text-white/80 leading-relaxed whitespace-pre-wrap">
                      {report.work_performed || 'No se detalló el trabajo en este reporte.'}
                    </p>
                  </div>
                </div>
                <div className="space-y-2">
          ...
          <div className="px-4 py-16 text-center">
            <div className="flex flex-col items-center gap-4 opacity-20">
              ...
              <p className="text-xs font-serif italic tracking-widest uppercase">No hay reportes disponibles</p>
            </div>
          </div>
        )}
      </div>
