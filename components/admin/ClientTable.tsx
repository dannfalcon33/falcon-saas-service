...
interface ClientTableProps {
  ...
  onEdit: (client: ClientWithSub) => void;
  onAdd: () => void;
}
...
export const ClientTable = ({ initialClients, onEdit, onAdd }: ClientTableProps) => {
  ...
  const handleInvite = async (id: string) => {
    ...
    try {
      ...
      alert('Error al enviar invitación: ' + getErrorMessage(err));
    } finally {
      setIsInviting(null);
    }
  };

  const getActiveSubscription = (client: ClientWithSub) => {
    return client.subscriptions?.find((sub) => sub.status === 'active');
  };

  const renderAccessState = (client: ClientWithSub) => {
    if (client.owner_profile_id) {
      return (
    ...
    return (
      <button
        ...
        className="flex items-center gap-2 text-[#3D7BFF] hover:text-[#3D7BFF]/80 transition-all font-black text-[10px] uppercase tracking-widest disabled:opacity-60"
      >
  ...
  const renderActionButtons = (client: ClientWithSub) => (
  ...
  return (
    <div className="bg-white/[0.03] border border-white/5 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden backdrop-blur-xl">
      <div className="p-4 md:p-8 border-b border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
        <div>
          <h2 className="text-xl md:text-2xl font-serif font-bold text-white tracking-tight">Directorio de Clientes</h2>
          <p className="text-[10px] md:text-xs text-[#8A9199] font-medium italic mt-1 font-serif tracking-widest uppercase opacity-80 leading-relaxed">
            Gestión proactiva de cuentas y datos corporativos.
          </p>
        </div>
        ...
        <div className="w-full md:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3 md:gap-4">
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/5 rounded-xl py-2.5 pl-10 pr-4 text-xs text-white outline-none focus:border-[#3D7BFF]/50 transition-all"
            />
          </div>
          <button
            onClick={onAdd}
            className="w-full sm:w-auto justify-center flex items-center gap-2 px-5 py-2.5 bg-[#3D7BFF] text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] hover:scale-[1.02] active:scale-95 transition-all shadow-[0_10px_20px_rgba(61,123,255,0.2)]"
          >
      ...
      <div className="md:hidden divide-y divide-white/5">
        {filteredClients.length === 0 ? (
          <div className="px-4 py-12 text-center">
            <p className="text-xs font-serif italic tracking-widest text-[#8A9199]/40 uppercase">No hay clientes que coincidan</p>
          </div>
        ) : (
          filteredClients.map((client) => {
            const activeSub = getActiveSubscription(client);
              ...
              <article key={client.id} className="p-4 space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-white tracking-tight truncate">{client.business_name}</p>
                      <p className="text-[10px] text-[#8A9199] font-medium truncate">{client.rif_or_id || 'Sin RIF'}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-white/80">
                    ...
                    <span className="text-xs font-bold truncate">{client.contact_name}</span>
                  </div>
                  <div className="flex items-center gap-2 text-[#8A9199]">
                    <Mail className="w-3 h-3" />
                    <span className="text-[11px] font-medium truncate">{client.main_email}</span>
                  </div>
                </div>

                <div className="p-3 rounded-xl border border-white/5 bg-black/20 space-y-2">
                  <p className="text-[10px] font-black tracking-widest uppercase text-white/30">Suscripción</p>
                  {activeSub ? (
                    <div>
                      <p className="text-xs font-bold text-white uppercase tracking-tighter">{activeSub.plan?.name}</p>
                      <p className="text-[10px] text-emerald-500 font-black italic">
                        Vence: {isMounted ? new Date(activeSub.end_date).toLocaleDateString() : '...'}
                      </p>
                    </div>
                  ) : (
                ...
                <div className="p-3 rounded-xl border border-white/5 bg-black/20 space-y-2">
                  <p className="text-[10px] font-black tracking-widest uppercase text-white/30">Acceso Dashboard</p>
                  {renderAccessState(client)}
                </div>

                <div className="pt-2 border-t border-white/5">{renderActionButtons(client)}</div>
              </article>
            );
          })
        )}
              ...
              filteredClients.map((client) => {
                ...
                return (
                  <tr key={client.id} className="group hover:bg-white/[0.01] transition-colors">
                    ...
                    <td className="px-8 py-6">{renderAccessState(client)}</td>
                    <td className="px-8 py-6">
                      <StatusBadge status={client.status} />
                    </td>
                    <td className="px-8 py-6">{renderActionButtons(client)}</td>
                  </tr>
                );
              })
            )}
