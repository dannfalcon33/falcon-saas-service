...
export const VisitTable = ({ initialVisits, initialRequests }: VisitTableProps) => {
  ...
  const handleUpdateVisitStatus = async (visitId: string, status: VisitStatus) => {
    ...
    setVisits((current) =>
      current.map((visit) => (visit.id === visitId ? { ...visit, status } : visit))
    );
    router.refresh();
    return true;
  };
      ...
      <div className="bg-white/[0.03] border border-white/5 rounded-[2.5rem] overflow-hidden backdrop-blur-xl">
        <div className="p-8 border-b border-white/5">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-[#3D7BFF]/10 text-[#3D7BFF] flex items-center justify-center border border-[#3D7BFF]/20">
              <ClipboardList className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-2xl font-serif font-bold text-white tracking-tight">Solicitudes de Visita Pendientes</h2>
              <p className="text-xs text-[#8A9199] font-medium italic mt-1 font-serif tracking-widest uppercase opacity-80 leading-relaxed">
        ...
        <div className="divide-y divide-white/5">
          ...
          {pendingRequests.map((request) => (
            <div key={request.id} className="px-8 py-6 flex flex-col lg:flex-row gap-6 lg:items-center lg:justify-between">
              <div className="space-y-2 max-w-3xl">
                <div className="flex items-center gap-3">
                  <p className="text-sm font-bold text-white">{request.client?.business_name}</p>
                ...
                <p className="text-xs text-[#8A9199] italic leading-relaxed line-clamp-2">{request.description}</p>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="px-5 py-3 bg-[#3D7BFF] text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] hover:scale-105 active:scale-95 transition-all"
                >
                  Programar
                </button>
                <button
                  onClick={() => openRejectModal(request)}
                  className="px-5 py-3 bg-white/5 border border-white/10 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] hover:border-red-500/40 hover:text-red-400 transition-all"
                >
                  Rechazar
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 md:p-6 backdrop-blur-3xl bg-black/60">
          <div className="bg-[#0B1622] border border-white/10 w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSchedule} className="p-8 space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-white/40">Descripción</label>
                <textarea
                  rows={3}
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 md:p-6 backdrop-blur-3xl bg-black/60">
          <div className="bg-[#0B1622] border border-white/10 w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleReject} className="p-8 space-y-5">
              <p className="text-xs text-[#8A9199] italic">Puedes dejar una nota visible para auditoría interna.</p>
              <textarea
                rows={4}
                value={rejectNotes}
